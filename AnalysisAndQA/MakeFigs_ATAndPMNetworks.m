...
[WorkingTable,PropRedNetList(:,1),RedNetNames] = MakeFigs_FxnToComputeDegreePerNetwork_ATAndPMNetworks(ResTbl(:,1:2));
...
C=linspecer(2);
RedNetNames2={"Vis","SoMat","DorsAttn","SalVentAttn","Limbic","Control","DMN","TempPar","Subcor","AT","PM"}';
CC(1:11,:)=[C(1,:);C(1,:);C(1,:);C(1,:);C(1,:);C(1,:);C(1,:);C(1,:);C(1,:);C(1,:);C(1,:)];
figure()
circularGraph(A,'Label',RedNetNames2,'Colormap',CC)
set(gca,'FontSize',20)
set(gca,'FontName','Arial')
saveas(gcf,'CirclePlot_PreVsPost_ATPM.png');
close all
...
[WorkingTable,PropRedNetList(:,2),RedNetNames] = MakeFigs_FxnToComputeDegreePerNetwork_ATAndPMNetworks(tmpResTbl(:,1:2));
...
CC(1:11,:)=[C(2,:);C(2,:);C(2,:);C(2,:);C(2,:);C(2,:);C(2,:);C(2,:);C(2,:);C(2,:);C(2,:)];
figure()
circularGraph(A,'Label',RedNetNames2,'Colormap',CC)
set(gca,'FontSize',20)
set(gca,'FontName','Arial')
saveas(gcf,'CirclePlot_PostVsTI11_ATPM.png');
close all
...
[WorkingTable,PropRedNetList(:,3),RedNetNames] = MakeFigs_FxnToComputeDegreePerNetwork_ATAndPMNetworks(tmpResTbl(:,1:2));
...
set(gca,'FontSize',20)
set(gca,'FontName','Arial')
saveas(gcf,'CirclePlot_PostVsTI13_ATPM.png');
close all
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
close all
spider_plot(PropRedNetList','AxesLabels',RedNetNames2,'AxesFont', 'Arial')
legend('PreVsPost', 'PostvsTI11', 'PostVsTI13','Location','southoutside')
...
for ii=1:3
figure()
pie(PropRedNetList(:,ii)',RedNetNames)
end
...
%%
% What is the relative weight of each network?
...
for ii=1:11
Weight(ii,1)=sum(MassiveNetworkTbl(:,2)==ii)/218;
end